...
parser.add_argument('--hidden', type=int, default=8,  # 16
...
parser.add_argument('--model',type=str,default='fadap',choices=['fadap','fadap_nosequence',])
...
if args.dataset == 'Sichuan_one':
    # 加载数据集
    edge_index, oneweek, twoweek ,threeweek, month, id_feature, labels, idx_train, idx_val, idx_test = load_Sichuan_1(args)
    ...
    model = FAdaLayer_noAF(arg=args,g=g,in_dim=args.hidden*4+2,dropout_rate=args.dropout,num_layer=args.layers,nclass=labels.max().item()+1)
    if args.cuda:
    ...
    def train(epoch,optimizer,oneweek, twoweek, threeweek, month, id_feature ,patience,loss_gamma):
        ...
        model.train()
        optimizer.zero_grad()
        output,_ = model((oneweek, twoweek, threeweek, month, id_feature))

        # loss_indiv=F.nll_loss(_[idx_train],labels[idx_train])
    ...
    def test():
        ...
        if args.dataset != 'citeseer':
            loss_test = F.nll_loss(output[idx_test], labels[idx_test])
            acc_test = accuracy(output[idx_test], labels[idx_test])
            acc_test_lstm = accuracy(_[idx_test], labels[idx_test])
            output=output.detach().cpu().numpy()
            # _=_.detach().cpu().numpy()
            labels=labels.cpu().numpy()
            y_pre=np.argmax(output,axis=1)
            # y_pre_lstm=np.argmax(_,axis=1)
            y_score=output[:,-1]
            # y_score_lstm=_[:,-1]
            auc_test = roc_auc_score(labels[idx_test],y_score[idx_test],average='macro')
            recall_test= recall_score(labels[idx_test],y_pre[idx_test],average='macro')
    ...
    if args.train:
        for epoch in range(args.epoches):
            train(epoch,optimier,oneweek, twoweek, threeweek, month, id_feature ,args.patience,args.loss_gamma)
        print("Optimization Finished!")
