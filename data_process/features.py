...
def time_gap(vol_list):
    if vol_list.empty:
        return 0
    days=[g for n,g in vol_list.groupby(pd.Grouper(key='start_datetime',freq='D'))]
    gaps=[]
...
def recall_rate(vol_list):
    ...
    for idx,row in vol_list.iterrows():
        if row['calltype_id']==1:
            ...
            call_back=vol_list.loc[(vol_list['start_datetime']>row['start_datetime'])&(vol_list['calltype_id']==2)&(vol_list['opposite_no_m']==row['opposite_no_m'])]
            if len(call_back)>0:
                n_call_back+=1
    if n_call_to==0:return 0
...
def energy_dispersion(vol_list):
    ...
    if vol_list.empty:
        return (0,0)
    vol_list['counting']=vol_list.groupby('opposite_no_m')['opposite_no_m'].transform('count') #按照对方的电话分组
    vol_list['counting']=vol_list['counting']/len(vol_list)
    each_one=list((vol_list.drop_duplicates(subset='opposite_no_m'))['counting'])
